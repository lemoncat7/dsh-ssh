...
简体中文文档见本文件；English docs: [README.md](./README.md)。

## 语言

全部界面文案均为双语，跟随 DSH 显示语言（设置 → 通用 → 语言）在 **English** 与 **中文** 间切换。组件内固化的是英文源文案，中文经 DSH locale 服务绑定 `src/locales/zh.ts` 字典；字典缺项时自动回退英文，切换语言不会出现空白文案。AI 工具描述与主机端错误信息在 DSH 启动时解析一次，切换语言后需重启 DSH 才会生效。
## 安装
