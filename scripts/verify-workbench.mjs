  ...
  for (const width of [375, 1024, 1600]) for (const colorScheme of ['light', 'dark']) {
    ...
    await page.route('**/ssh-local/v1/**', async route => {
      ...
      if (path === '/terminals' && method === 'POST') {
        ...
        if (delayNext) { delayNext = false; await new Promise(resolve => { releaseOpen = resolve }) }
        return json({ id })
      }
    ...
    assert.equal(await page.locator('#ssh-terminal-a-7').evaluate(node => node.style.order), '0')
    ...
    if (width < 820) await page.getByRole('button', { name: '主机', exact: true }).click()
    await page.getByTitle('选择并展开 开发主机', { exact: true }).click()
    await page.getByRole('button', { name: '关闭全部终端', exact: true }).click()
    await page.getByRole('button', { name: '取消', exact: true }).click()
    assert.equal(await page.locator('.dsh-ssh-terminal-slot:visible').count(), 4, 'cancel must preserve panes')
    assert.equal(closed.length, 2)
    await page.getByRole('button', { name: '关闭全部终端', exact: true }).click()
    await page.getByRole('button', { name: '确认关闭全部', exact: true }).click()
    await page.waitForTimeout(120)
    assert.equal(await page.locator('.dsh-ssh-host-page:visible .dsh-ssh-terminal-slot').count(), 0)
    assert.equal(closed.length, 4, 'close all includes hidden terminals on every host')
    assert.equal(await page.locator('#ssh-terminal-b-1').count(), 0, 'other host tabs must also close')
    assert.equal(await page.getByRole('button', { name: '关闭全部终端', exact: true }).isDisabled(), true)
    await page.getByRole('button', { name: '新建终端', exact: true }).click()
    assert.equal(await page.getByRole('tab', { name: '终端 1', exact: true }).count(), 1)
    assert.equal(await page.locator('#ssh-terminal-a-8').getAttribute('aria-label'), '终端 1', 'reused label must have a fresh internal identity')
    await page.evaluate(() => window.unmountWorkspace())
    await page.waitForTimeout(120)
    assert.equal(closed.length, 4, 'leaving after closing all must not double-close terminals')
