...
侧栏入口名为「远端」，通过官方 `sidebar.footer.action` 注册。浏览器兼容层只把这个入口锚定到官方 Workspace 区域上方；它不会替换 `sidebar.workspaces`，锚点失效时会自动留在官方 Footer。远端标题右侧按钮与折叠栏图标负责开关 SSH 右侧栏，列表底部的「新建 SSH 会话」负责进入连接创建流程。
