...
- 重构 SSH 工作区和活动侧栏的视觉层级，统一主机、SFTP、多终端与弹窗控件样式。
- 终端外壳与 xterm 测量视口分层，统一字体、行高、尺寸同步和字体加载后的 Fit，修复活动终端底行裁切。
- 多终端切换、主机列表、SFTP、文件预览和弹窗使用一致的轻量滚动条，不再继承浏览器或外部主题的滚动条颜色。
- 统一按钮原生外观、下拉箭头和键盘焦点环，同时保留复选框、单选框和数字步进器的系统级可访问交互。
- 接入共享插件工作区互斥机制，切换知识库、SSH 等工作区时自动关闭旧工作区，避免会话插槽冲突。
