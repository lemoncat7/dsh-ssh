...
- 会话标题栏提供右侧栏开关；目录页使用真实 SFTP 浏览远端文件，终端页使用单一只读终端画面观察 AI 操作
- 插件服务与 Web 客户端兼容 Windows、macOS 和 Linux，不依赖本机 `ssh` 或 `sftp` 命令
- 首次连接主机指纹确认、输出上限、命令超时和公开端口绑定保护

## 界面

侧栏入口名为「远端」，通过官方 `sidebar.footer.action` 注册。浏览器兼容层只把这个入口锚定到官方 Workspace 区域上方；它不会替换 `sidebar.workspaces`，锚点失效时会自动留在官方 Footer。

管理工作区分成三部分：

1. 左侧连接列表：搜索、选择和凭据状态。
2. 中间工作区：终端、端口转发和全局设置。
3. 右侧会话注入检查器：选择当前对话可用的主机、权限级别，以及是否自动允许权限范围内的 SSH 操作。

完成会话注入后，聊天标题栏最右侧的 Utilities 区域会出现侧栏图标。仅执行命令权限显示 SFTP 目录；终端控制权限同时显示 SFTP 目录和 AI 终端。多个终端通过紧凑标签切换，运行中的终端支持键盘输入。进入 SFTP 子目录时，该目录会同步成为后续 `ssh_exec` 与新终端的工作目录。
