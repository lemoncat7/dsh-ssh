...
test('shared interactive surface contract owns themed hover and selection states', async () => {
  ...
  assert.match(sftpSource, /deletion=\{\{ locationName: t\("Local session"\), locationKind: 'local', remove \}\}/)
...
test('profile metadata pickers, password visibility, and host groups use owned accessible controls', async () => {
  ...
  assert.match(profileSource, /<SuggestionInput ariaLabel=\{t\("Host group"\)\}/)
  assert.match(profileSource, /<SuggestionInput ariaLabel=\{t\("Host tags"\)\} multiple/)
  assert.match(ftpSource, /<SuggestionInput ariaLabel=\{t\("FTP groups"\)\}/)
  assert.match(ftpSource, /<SuggestionInput ariaLabel=\{t\("FTP tags"\)\} multiple/)
  assert.match(uiSource, /role="combobox"/)
  assert.match(uiSource, /role="listbox"/)
  assert.match(uiSource, /aria-label=\{visible \? t\("Hide password"\) : t\("Show password"\)\}/)
