...
test('shared interactive surface contract owns themed hover and selection states', async () => {
  ...
  assert.match(css, /\.dsh-ssh-workspace \.dsh-ssh-transfer-tab:is\(\.is-active, \.is-active:hover\)\s*\{[\s\S]*?background-color:\s*var\(--ssh-tab-selected\);[\s\S]*?box-shadow:/)
  ...
  assert.match(sftpSource, /deletion=\{\{ locationName: t\("sftp-client.localSession"\), locationKind: 'local', remove \}\}/)
  assert.match(remoteTreeSource, /dsh-ssh-tree-mount dsh-ssh-context-action/)
  assert.match(css, /\[data-ssh-interactive="row"\]:hover\s*\{[\s\S]*?background-color:\s*var\(--ssh-hover\);/)
  assert.doesNotMatch(css, /background:\s*var\(--ssh-(?:hover|selected|tab-selected)\);\s*background-image:\s*none;/)
})
...
test('all SSH workspaces share one cool-charcoal material and typography contract', async () => {
  ...
  assert.match(css, /--ssh-hover:\s*rgba\(60, 60, 67, 0\.12\);/)
  assert.match(css, /--ssh-selected:\s*rgba\(60, 60, 67, 0\.16\);/)
  assert.match(css, /--ssh-chrome-surface:\s*var\(--ssh-panel\);/)
  assert.match(css, /--ssh-card-surface:/)
  assert.match(css, /--ssh-border-strong:\s*rgba\(60, 60, 67, 0\.24\);/)
  ...
  assert.match(css, /body\[data-ds-dark-theme\][\s\S]*--ssh-hover:\s*rgb\(105 182 186 \/ 13%\);/)
  assert.match(css, /body\[data-ds-dark-theme\][\s\S]*--ssh-border-strong:\s*rgb\(184 204 205 \/ 18%\);/)
...
test('profile metadata pickers, password visibility, and host groups use owned accessible controls', async () => {
  ...
  assert.match(profileSource, /<SuggestionInput ariaLabel=\{t\("profile-editor.hostGroup"\)\}/)
  assert.match(profileSource, /<SuggestionInput ariaLabel=\{t\("profile-editor.hostTags"\)\}\s+multiple/)
  assert.match(ftpSource, /<SuggestionInput ariaLabel=\{t\("ftp-profile-editor.ftpGroups"\)\}/)
  assert.match(ftpSource, /<SuggestionInput ariaLabel=\{t\("ftp-profile-editor.ftpTags"\)\}\s+multiple/)
  assert.match(uiSource, /role="combobox"/)
  assert.match(uiSource, /role="listbox"/)
  assert.match(uiSource, /aria-label=\{visible \? t\("ui-components.hidePassword"\) : t\("ui-components.showPassword"\)\}/)
