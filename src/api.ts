...
import {
  normalizeForwardDraft, normalizeProfileDraft, normalizeSecrets,
  type ForwardRule, type SessionInjection, type SshProfile,
} from './domain.js'
...
async function dispatch(req: IncomingMessage, res: ServerResponse, prefix: string, runtime: SshApiRuntime): Promise<void> {
  ...
  if (segments[0] === 'profiles') {
    ...
    if (method === 'POST' && segments.length === 1) {
      ...
      await runtime.credentials.replace(profile.id, secrets)
      try { await runtime.store.update(state => { state.profiles.push(profile) }) }
      catch (error) { await runtime.credentials.delete(profile.id).catch(() => {}); throw error }
...
async function profileView(runtime: SshApiRuntime, profile: SshProfile): Promise<unknown> {
  ...
  return { ...profile, credential: { ...credential, configured: requiredField === undefined || credential.fields.includes(requiredField) } }
}
