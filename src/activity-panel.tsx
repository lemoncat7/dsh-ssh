...
export function SshActivityPanel(props: DetailsProps & { controller: ActivityController }): JSX.Element {
  ...
  return <section className="dsh-ssh-activity-panel" aria-label={t("SSH Activity")}>
    <header className="dsh-ssh-activity-header">
      <div className="dsh-ssh-activity-title">
        ...
        <span><strong>{t("SSH activity")}</strong><small>Current session · {shortId(sessionId)}</small></span>
      </div>
      <button type="button" className="dsh-ssh-icon-button" aria-label={t("Close SSH Activity")} onClick={() => props.controller.close(sessionId)}><IconCloseOutline16 size={16} /></button>
    </header>
    {activity && <nav className="dsh-ssh-activity-tabs" aria-label={t("SSH activity view")}>
      <button type="button" data-ssh-interactive="choice" className={view === 'local-directory' ? 'is-active' : ''} aria-pressed={view === 'local-directory'} onClick={() => props.controller.open(sessionId, undefined, 'local-directory')}><IconFolderOpenOutline16 size={16} />{t("Session directory")}</button>
      {activity.profiles.length > 0 && <button type="button" data-ssh-interactive="choice" className={view === 'remote-directory' ? 'is-active' : ''} aria-pressed={view === 'remote-directory'} onClick={() => props.controller.open(sessionId, undefined, 'remote-directory')}><ServerGlyph />{t("Remote directory")}</button>}
    ...
    <div className="dsh-ssh-activity-body">
      {activity === undefined ? <p className="dsh-ssh-activity-state" role="status">{t("Reading SSH sessions…")}</p>
...
function RemoteDirectoryActivity({ sessionId, profiles, selectedProfileId, onProfile, onSaved }: { sessionId: string; profiles: ActivityProfileView[]; selectedProfileId?: string | undefined; onProfile(id: string): void; onSaved(): Promise<void> }): JSX.Element {
  ...
  if (profile === undefined) return <div className="dsh-ssh-activity-empty"><IconFolderOpenOutline16 size={22} /><strong>{t("No browsable remotes")}</strong><p>{t("Allow the current session to access a host in the SSH panel first.")}</p></div>
...
function TerminalActivity({ sessionId, terminals, onClosed }: { sessionId: string; terminals: ActivityTerminalView[]; onClosed(): Promise<void> }): JSX.Element {
  ...
  if (terminal === undefined) return <div className="dsh-ssh-activity-empty"><IconCodeOutline16 size={22} /><strong>{t("No terminals open yet")}</strong><p>{t("Terminals opened by the AI appear here directly.")}</p></div>
  ...
  const closeLabel = terminal.status.kind === 'running' ? t("End") : t("Remove")

  return <div className="dsh-ssh-terminal-workbench">
    {terminals.length > 1 && <nav className="dsh-ssh-terminal-switcher dsh-ssh-scroll-surface" aria-label={t("SSH terminal")}>{terminals.map((item, index) => {
      const label = item.name || tx`Terminal ${index + 1}`
    ...
    <div className="dsh-ssh-terminal-observer">
      <div className="dsh-ssh-terminal-observer-heading"><span><strong>{terminal.name}</strong><small>{terminal.cwd}</small></span><div className="dsh-ssh-terminal-observer-actions"><span className={`dsh-ssh-terminal-state is-${terminal.status.kind}`}>{terminal.status.kind === 'running' ? t("Running") : t("Exited")}</span><button type="button" className="dsh-ssh-terminal-close" disabled={closingId !== undefined} aria-label={tx`${closeLabel}Terminal ${terminal.name}`} title={terminal.status.kind === 'running' ? t("End and close this terminal") : t("Remove this terminal from the activity panel")} onClick={() => { void closeSelectedTerminal() }}><IconCloseOutline16 size={14} /><span>{closingId === terminal.terminalId ? t("Processing") : closeLabel}</span></button></div></div>
...
function InteractiveTerminal({ sessionId, terminal: activity, onError }: { sessionId: string; terminal: ActivityTerminalView; onError(value: string | undefined): void }): JSX.Element {
  ...
  return <div className="dsh-ssh-terminal-screen" aria-label={t("Interactive SSH terminal")}><div ref={hostRef} className="dsh-ssh-terminal-viewport" /></div>
}
