...
import {
  ApiError, api, loadActivity, loadForwards, loadInjection, loadProfiles, loadSftpDirectory, profileAddress, updateActivityDirectory,
  type ActivityProfileView, type ActivityTerminalView, type ActivityView, type ForwardStatus, type ForwardView,
  type InjectionView, type ProfileView, type SettingsView, type SftpDirectoryView,
} from './client-api.js'
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.slots.inject('conversation.session.header.actions', () => ctx.slots.register({
    name: 'conversation.session.header.actions', id: 'ssh-activity', order: 60,
    ...
    <div className="dsh-ssh-activity-body">
      {activity === undefined ? <p className="dsh-ssh-activity-state">正在读取 SSH 会话…</p>
        : view === 'directory' ? <DirectoryActivity sessionId={sessionId} profiles={activity.profiles} onSaved={refresh} />
          : <TerminalActivity terminals={activity.terminals} />}
    ...
    <div className="dsh-ssh-sftp-table" aria-busy={loading}>
      ...
      {loading && directory === undefined ? <p className="dsh-ssh-sftp-state">正在读取远端目录…</p>
        : directory?.entries.length === 0 ? <p className="dsh-ssh-sftp-state">此目录为空</p>
          : directory?.entries.map(entry => <button type="button" className={`dsh-ssh-sftp-row is-${entry.kind}`} key={entry.path} disabled={entry.kind !== 'directory'} onDoubleClick={() => { if (entry.kind === 'directory') void browse(entry.path, true) }} onClick={() => { if (entry.kind === 'directory') void browse(entry.path, true) }}>
...
function TerminalActivity({ terminals }: { terminals: ActivityTerminalView[] }): JSX.Element {
  const terminal = [...terminals].reverse().find(item => item.status.kind === 'running') ?? terminals.at(-1)
  ...
  return <div className="dsh-ssh-terminal-observer">
    ...
    <ReadOnlyTerminal value={terminal.scrollback} />
  </div>
}

function ReadOnlyTerminal({ value }: { value: string }): JSX.Element {
  ...
  useEffect(() => {
    ...
    const terminal = new Terminal({
      disableStdin: true, cursorBlink: false, cursorInactiveStyle: 'none', fontSize: 11, lineHeight: 1.35, scrollback: 4000,
    ...
    terminal.loadAddon(fit); terminal.open(host); fit.fit(); terminalRef.current = terminal
    const resize = new ResizeObserver(() => fit.fit())
    resize.observe(host)
    return () => { resize.disconnect(); terminal.dispose(); terminalRef.current = undefined; previousRef.current = '' }
  }, [])
  useEffect(() => {
    const terminal = terminalRef.current
    if (terminal === undefined) return
    const previous = previousRef.current
    if (value.startsWith(previous)) terminal.write(value.slice(previous.length))
    else { terminal.reset(); terminal.write(value) }
    previousRef.current = value
    terminal.scrollToBottom()
  }, [value])
  return <div ref={hostRef} className="dsh-ssh-terminal-screen" aria-label="SSH 终端输出" />
}
...
function RemoteWorkspace(props: ConversationProps & { controller: RemoteController }): JSX.Element {
  ...
  const [view, setView] = useState<'terminal' | 'forwards' | 'settings'>('terminal')
  ...
  return <main className="dsh-ssh-workspace">
    ...
    {editing !== undefined && <ProfileEditor profile={editing === 'new' ? undefined : editing} profiles={profiles} onClose={() => setEditing(undefined)} onSaved={() => { setEditing(undefined); setRefreshKey(value => value + 1) }} />}
  </main>
}
...
function ProfileEditor({ profile, profiles, onClose, onSaved }: { profile?: ProfileView | undefined; profiles: ProfileView[]; onClose(): void; onSaved(): void }): JSX.Element {
  const [form, setForm] = useState(() => ({
    ...
    proxyType: profile?.proxy.type ?? 'none', proxyHost: profile?.proxy.type === 'http' || profile?.proxy.type === 'socks5' ? profile.proxy.host : '', proxyPort: profile?.proxy.type === 'http' || profile?.proxy.type === 'socks5' ? String(profile.proxy.port) : '1080', proxyUsername: profile?.proxy.type === 'http' || profile?.proxy.type === 'socks5' ? profile.proxy.username ?? '' : '', jumpProfileId: profile?.proxy.type === 'jump' ? profile.proxy.profileId : '', tags: profile?.tags.join(', ') ?? '',
    password: '', privateKey: '', passphrase: '', proxyPassword: '',
  }))
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState<string>()
  const field = (name: keyof typeof form) => ({ value: form[name], onChange: (event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => setForm(value => ({ ...value, [name]: event.target.value })) })
  const submit = async (event: FormEvent): Promise<void> => {
    event.preventDefault(); setSaving(true); setError(undefined)
    const proxy = form.proxyType === 'none' ? { type: 'none' }
      : form.proxyType === 'jump' ? { type: 'jump', profileId: form.jumpProfileId }
    ...
    const payload = {
      profile: { name: form.name, host: form.host, port: Number(form.port), username: form.username, authType: form.authType, hostFingerprint: profile?.hostFingerprint, proxy, keepAliveIntervalMs: profile?.keepAliveIntervalMs ?? 15000, connectTimeoutMs: profile?.connectTimeoutMs ?? 15000, terminalType: profile?.terminalType ?? 'xterm-256color', tags: form.tags.split(',').map(item => item.trim()).filter(Boolean) },
  ...
  return <Dialog title={profile === undefined ? '新建 SSH 连接' : `编辑 ${profile.name}`} subtitle="凭据保存后不会回显" onClose={onClose}>
    <form className="dsh-ssh-form" onSubmit={event => { void submit(event) }}>
      ...
      {form.authType === 'password' && <Field label="密码" hint={profile?.credential.fields.includes('password') ? '已保存；留空保持不变' : '保存后不可读回'}><input type="password" autoComplete="new-password" placeholder={profile?.credential.fields.includes('password') ? '••••••••' : ''} {...field('password')} /></Field>}
      {form.authType === 'private-key' && <><Field label="私钥" hint={profile?.credential.fields.includes('privateKey') ? '已保存；留空保持不变' : '粘贴 OpenSSH/PEM 私钥'}><textarea rows={5} spellCheck={false} {...field('privateKey')} /></Field><Field label="私钥口令"><input type="password" autoComplete="new-password" {...field('passphrase')} /></Field></>}
      ...
      {form.proxyType === 'jump' && <Field label="跳板连接"><select required {...field('jumpProfileId')}><option value="">选择已有连接</option>{profiles.filter(item => item.id !== profile?.id).map(item => <option value={item.id} key={item.id}>{item.name} · {item.host}</option>)}</select></Field>}
      {error && <p className="dsh-ssh-inline-error" role="alert">{error}</p>}
