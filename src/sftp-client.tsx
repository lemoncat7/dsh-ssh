...
export function LocalWorkspaceBrowser({ sessionId }: { sessionId: string }): JSX.Element {
  ...
  return <SftpExplorer initialPath="" loadDirectory={loadDirectory} loadPreview={loadPreview} fileUrl={fileUrl} deletion={{ locationName: t("Local session"), locationKind: 'local', remove }} />
}
...
export function ActivitySftpBrowser({ sessionId, profile, profiles, onProfile, onSaved }: { sessionId: string; profile: ActivityProfileView; profiles: ActivityProfileView[]; onProfile(id: string): void; onSaved(): Promise<void> }): JSX.Element {
  ...
  const header = <div className="dsh-ssh-sftp-hostbar">
    ...
    <label><span className="sr-only">{t("Select remote host")}</span><select value={profile.id} onChange={event => onProfile(event.target.value)}>{profiles.map(item => <option value={item.id} key={item.id}>{item.name}</option>)}</select><small>{profile.username}@{profile.host}:{profile.port}</small></label>
  </div>
...
function SftpExplorer({ initialPath, header, workspace = false, loadDirectory, loadPreview, fileUrl, uploadFile, operations, deletion }: SftpExplorerProps): JSX.Element {
  ...
  const uploadFiles = async (files: File[], targetDirectory = directory?.path, overwriteFirst = false): Promise<void> => {
    ...
    if (accepted.length === 0) { setError(t("A single file cannot exceed 512 MB.")); return }
    ...
    if (oversized > 0) setError(tx`Skipped ${oversized} files larger than 512 MB`)
  }
  const canDropFiles = uploadFile !== undefined && directory !== undefined && uploading === undefined && pendingOverwrite === undefined
  const isFileDrag = (event: DragEvent<HTMLDivElement>): boolean => Array.from(event.dataTransfer.types).includes(t("Files"))
  ...
  const applyRemoteDrop = async (source: RemoteFilesDragPayload, destinationDirectory: string): Promise<void> => {
    ...
    try {
      ...
      if (result.operation === 'move') { setOperationMessage(t("Moved")); await browse(directory?.path ?? destinationDirectory, false) }
      if (result.operation === 'copy') setOperationMessage(t("Added to the transfer queue. See progress in File Transfers."))
  ...
  return <div className={`dsh-ssh-sftp${workspace ? ' is-workspace' : ''}${draggingFiles ? ' is-dragging-files' : ''}`} onDragEnter={handleDragEnter} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
    ...
    {openedFile ? <SftpFilePreview entry={openedFile} loadPreview={loadPreview} fileUrl={fileUrl} onBack={() => setOpenedFile(undefined)} /> : <>
      <form className={`dsh-ssh-sftp-pathbar${uploadFile === undefined ? '' : ' has-upload'}`} onSubmit={submit}>
        <button type="button" aria-label={t("Go to parent directory")} title={t("Go to parent directory")} disabled={directory?.parent == null || loading} onClick={() => { if (directory?.parent) void browse(directory.parent, true) }}><IconChevronLeftOutline14 size={14} /></button>
        <input aria-label={t("Current remote directory")} value={path} spellCheck={false} onChange={event => setPath(event.target.value)} />
        <button type="button" aria-label={t("Refresh directory")} title={t("Refresh directory")} disabled={loading} onClick={() => { void browse(directory?.path ?? path, false) }}><IconRefreshOutline16 size={15} /></button>
        {uploadFile !== undefined && <><input ref={fileInputRef} className="sr-only" type="file" multiple tabIndex={-1} onChange={event => { const files = Array.from(event.target.files ?? []); event.target.value = ''; if (files.length > 0) void uploadFiles(files) }} /><button type="button" className="dsh-ssh-sftp-upload-button" disabled={directory === undefined || uploading !== undefined || pendingOverwrite !== undefined} onClick={() => fileInputRef.current?.click()}><IconSendOutline14 size={14} />{uploading === undefined ? t("Upload") : t("Uploading")}</button></>}
      </form>
      {pendingOverwrite !== undefined && <div className="dsh-ssh-upload-conflict" role="alert"><span><strong>{t("A file with the same name exists")}</strong><small>{pendingOverwrite.file.name}</small></span><span><button type="button" onClick={() => { const pending = pendingOverwrite; setPendingOverwrite(undefined); void uploadFiles(pending.remaining, pending.directory) }}>{t("Skip")}</button><button type="button" className="is-primary" disabled={uploading !== undefined} onClick={() => { const pending = pendingOverwrite; void uploadFiles([pending.file, ...pending.remaining], pending.directory, true) }}>{t("Overwrite")}</button></span></div>}
      {operationMessage && <div className="dsh-ssh-sftp-operation-status" role="status"><span>{operationMessage}</span><button type="button" aria-label={t("Dismiss notice")} onClick={() => setOperationMessage(undefined)}><IconCloseOutline16 size={14} /></button></div>}
      ...
      <div className="dsh-ssh-sftp-table dsh-ssh-scroll-surface" aria-busy={loading}>
        <div className="dsh-ssh-sftp-columns"><span>{t("Name")}</span><span>{t("Size")}</span><span>{t("Modified")}</span><span aria-hidden="true" /></div>
        {loading && directory === undefined ? <p className="dsh-ssh-sftp-state">{t("Reading remote directory…")}</p>
          : directory?.entries.length === 0 ? <p className="dsh-ssh-sftp-state">{t("This directory is empty")}</p>
            : directory?.entries.map(entry => {
              ...
              return <div role="row" tabIndex={0} aria-label={`${entry.name}${directoryEntry ? t(", directory, click to enter; accepts drag & drop") : t(", file, click to preview")}`} data-ssh-interactive="row" data-ssh-context-row draggable={operations !== undefined && (entry.kind === 'file' || directoryEntry)} className={`dsh-ssh-sftp-row is-${directoryEntry ? 'directory' : entry.kind}${remoteDropTarget === entry.path ? ' is-drop-target' : ''}`} key={entry.path}
              ...
              {deletion !== undefined && <button type="button" className="dsh-ssh-sftp-row-delete dsh-ssh-context-action" draggable={false} aria-label={tx`Delete ${entry.name}`} title={tx`Delete ${entry.name}`} onClick={event => { event.stopPropagation(); setDeleteTarget(entry) }}><IconTrashOutline16 size={14} /></button>}
            </div>})}
      </div>
    </>}
    {draggingFiles && <div className="dsh-ssh-sftp-dropzone" aria-hidden="true"><span><strong>{t("Release to upload")}</strong><small>Upload to {directory?.path ?? path}</small></span></div>}
...
function SftpFilePreview({ entry, loadPreview, fileUrl, onBack }: { entry: SftpEntryView; loadPreview(path: string): Promise<SftpFilePreviewView>; fileUrl(path: string, inline?: boolean): string; onBack(): void }): JSX.Element {
  ...
  return <><section className="dsh-ssh-file-preview">
    <header><button type="button" className="dsh-ssh-icon-button" aria-label={t("Back to directory")} title={t("Back to directory")} onClick={onBack}><IconChevronLeftOutline14 size={14} /></button><span className="dsh-ssh-file-preview-title"><strong title={entry.name}>{entry.name}</strong><small>{formatBytes(entry.size)}</small></span><span className="dsh-ssh-file-preview-actions"><a href={downloadUrl} aria-label={t("Download file")} title={t("Download file")}><IconDownloadOutline16 size={16} /></a><button type="button" aria-label={t("Zoom preview")} title={t("Zoom preview")} onClick={() => setExpanded(true)}><IconFullscreenOutline16 size={16} /></button></span></header>
  ...
  </section><Modal open={expanded} onClose={() => setExpanded(false)} title={tx`Preview ${entry.name}`} headless className="dsh-ssh-preview-modal"><section className="dsh-ssh-preview-modal-shell"><header><span><strong title={entry.name}>{entry.name}</strong><small>{formatBytes(entry.size)}</small></span><span className="dsh-ssh-file-preview-actions"><a href={downloadUrl} aria-label={t("Download file")} title={t("Download file")}><IconDownloadOutline16 size={16} /></a><button type="button" aria-label={t("Close preview")} title={t("Close preview")} onClick={() => setExpanded(false)}><IconCloseOutline16 size={16} /></button></span></header><div className="dsh-ssh-file-preview-body is-modal dsh-ssh-scroll-surface"><SftpPreviewContent entry={entry} preview={preview} error={error} fileUrl={fileUrl} downloadUrl={downloadUrl} /></div></section></Modal></>
}
...
function SftpPreviewContent({ entry, preview, error, fileUrl, downloadUrl }: { entry: SftpEntryView; preview: SftpFilePreviewView | undefined; error: string | undefined; fileUrl(path: string, inline?: boolean): string; downloadUrl: string }): JSX.Element {
  ...
  if (preview === undefined) return <p className="dsh-ssh-sftp-state">{t("Opening file…")}</p>
  if (preview.kind === 'text' && preview.mimeType === 'text/markdown') return <><article className="dsh-ssh-markdown-preview"><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ a: props => <a {...props} target="_blank" rel="noreferrer" /> }}>{preview.text || ''}</ReactMarkdown></article>{preview.truncated && <small>{t("File is large; showing the first 1 MB only. Download to view the full content.")}</small>}</>
  if (preview.kind === 'text') return <><pre>{preview.text || ''}</pre>{preview.truncated && <small>{t("File is large; showing the first 1 MB only. Download to view the full content.")}</small>}</>
  ...
  return <div className="dsh-ssh-file-binary"><IconDataOutline16 size={24} /><strong>{t("This file cannot be previewed directly")}</strong><p>{preview.mimeType}</p><a href={downloadUrl}><IconDownloadOutline16 size={16} />{t("Download file")}</a></div>
}
