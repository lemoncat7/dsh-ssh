...
export function Dialog({ title, subtitle, className, onClose, children }: DialogProps): JSX.Element {
  ...
  return <Modal open onClose={closeWithMotion} title={title} headless className={`dsh-ssh-dialog-modal${className === undefined ? '' : ` ${className}-modal`}`}>
    <section ref={surfaceRef} className={`dsh-ssh-dialog dsh-ssh-scroll-surface${className === undefined ? '' : ` ${className}`}`} aria-labelledby={titleId} aria-describedby={subtitle === undefined ? undefined : descriptionId} onClickCapture={captureClose}>
      <header><span><h2 id={titleId}>{title}</h2>{subtitle && <p id={descriptionId}>{subtitle}</p>}</span><button type="button" className="dsh-ssh-icon-button" onClick={closeWithMotion} aria-label={t("Close")}><IconCloseOutline16 size={16} /></button></header>
      {children}
    </section>
  </Modal>
}
...
export function SuggestionInput({ value, options, placeholder, maxLength, multiple = false, ariaLabel, onChange }: SuggestionInputProps): JSX.Element {
  ...
  const choose = (option: string): void => {
    if (!multiple) onChange(option)
      ...
      onChange(dedupeLabels([...(replaceDraft ? parts.slice(0, -1) : parts), option]).join(t(", ")))
    }
    setFilter('')
    setOpen(false)
  ...
  return <div ref={rootRef} className={`dsh-ssh-suggestion-input${open ? ' is-open' : ''}`}>
    ...
    <button
      ...
      aria-label={tx`${open ? t("Collapse") : t("Expand")}${ariaLabel}existing options`}
      aria-expanded={open}
      onClick={() => {
    ...
    {open && <div id={listId} className="dsh-ssh-suggestion-menu dsh-ssh-scroll-surface" role="listbox" aria-label={tx`${ariaLabel}existing options`}>
      {filtered.length === 0 ? <p>{normalizedOptions.length === 0 ? t("No existing options yet. Type directly.") : t("No matches. Type a new value.")}</p>
...
export function PasswordInput(props: Omit<InputHTMLAttributes<HTMLInputElement>, 'type'>): JSX.Element {
  ...
  return <div className="dsh-ssh-password-input">
    ...
    <button type="button" aria-label={visible ? t("Hide password") : t("Show password")} aria-pressed={visible} onClick={() => setVisible(current => !current)}>{visible ? t("Hide") : t("Show")}</button>
  </div>
}
...
export function EmptyState(): JSX.Element {
  ...
  return <div ref={surfaceRef} className="dsh-ssh-empty-state"><span><ServerGlyph /></span><h1>{t("Connect your first remote host")}</h1><p>{t("Use the add button next to “Hosts & projects” to save an SSH config, then open terminals, set up port forwards, and grant per-session access to the AI.")}</p></div>
}
