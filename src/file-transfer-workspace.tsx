...
export function FileTransferWorkspace({ ftpProfiles, vaultEntries, proxyEntries, access, onProfilesChanged }: { ftpProfiles: FtpProfileView[]; vaultEntries: VaultEntryView[]; proxyEntries: ProxyEntryView[]; access: SessionAccessState; onProfilesChanged(): void }): JSX.Element {
  ...
  return <div className="dsh-ssh-transfer-workspace">
    <header className="dsh-ssh-transfer-header">
      <div><h1>File Transfer</h1><p>Direct streaming transfers between FTP, FTPS, and SFTP</p></div>
      <div className="dsh-ssh-transfer-actions"><button type="button" data-ssh-interactive="control" className="dsh-ssh-secondary-button" onClick={() => setAccessOpen(true)}>Session access · {access.value?.fileEndpointIds.length ?? 0}</button><button type="button" data-ssh-interactive="control" className="dsh-ssh-secondary-button" onClick={() => setConnectionsOpen(true)}>FTP manager</button></div>
    </header>
    {error && <div className="dsh-ssh-banner is-error" role="alert"><span>{error}</span><button type="button" onClick={() => setError(undefined)} aria-label="Close"><IconCloseOutline16 size={16} /></button></div>}
    <div className="dsh-ssh-transfer-tabbar" role="tablist" aria-label="File transfer tasks page">
      <div className="dsh-ssh-transfer-tabs">{tabs.map(tab => <span data-ssh-interactive="choice" className={`dsh-ssh-transfer-tab${tab.id === active?.id ? ' is-active' : ''}`} key={tab.id}><button type="button" role="tab" aria-selected={tab.id === active?.id} onClick={() => setActiveTabId(tab.id)}>{tab.name}</button>{tabs.length > 1 && <button type="button" className="is-close" aria-label={`Close ${tab.name}`} onClick={() => closeTab(tab.id)}><IconCloseOutline16 size={13} /></button>}</span>)}<button type="button" data-ssh-interactive="control" className="is-add" aria-label="New transfer tasks page" onClick={addTab}><IconPlusOutline16 size={15} /></button></div>
      <div className="dsh-ssh-pane-layout" aria-label="Panes">{[2, 3, 4].map(count => <button key={count} type="button" data-ssh-interactive="choice" aria-pressed={active?.panes.length === count} className={active?.panes.length === count ? 'is-active' : ''} onClick={() => setPaneCount(count)}>{count} panes</button>)}</div>
    </div>
    {endpoints.length === 0 ? <div className="dsh-ssh-transfer-empty"><IconDataOutline16 size={24} /><strong>No file connections available yet</strong><p>Create an FTP/FTPS connection, or add an SSH host first to use SFTP.</p><button type="button" className="dsh-ssh-primary-button" onClick={() => setConnectionsOpen(true)}>New FTP connection</button></div>
...
function FileTransferPane({ pane, endpoints, destination, dragSource, refreshRevision, onChange, onTransfer, onExternalDrop, onDragSourceChange, onManageConnections }: { pane: PaneState; endpoints: FileEndpointView[]; destination?: PaneState; dragSource?: TransferDragSource | undefined; refreshRevision: number; onChange(patch: Partial<PaneState>): void; onTransfer(paths: string[], destination: PaneState): void; onExternalDrop(source: TransferDragSource, destinationDirectory: string): void; onDragSourceChange(source?: TransferDragSource): void; onManageConnections(): void }): JSX.Element {
  ...
  if (endpoint === undefined) return <section className="dsh-ssh-file-pane is-connections">
    <header><span className="dsh-ssh-pane-heading"><strong>Connection list</strong><small>Click a connection to browse remote files</small></span><button type="button" data-ssh-interactive="control" className="dsh-ssh-pane-manage-button" onClick={onManageConnections}>FTP manager</button></header>
    <div className="dsh-ssh-endpoint-list">{endpoints.map(item => <button type="button" data-ssh-interactive="row" key={item.id} className="dsh-ssh-endpoint-row" onClick={() => chooseEndpoint(item)}><span className={`dsh-ssh-protocol-badge is-${item.protocol}`}>{protocolShort(item.protocol)}</span><span><strong title={item.name}>{item.name}</strong><small title={item.address}>{item.address}</small></span><span><i className={`dsh-ssh-protocol-dot is-${item.protocol}`} aria-hidden="true" />{item.group ?? (item.kind === 'sftp' ? "SSH hosts" : "File connections")}</span></button>)}</div>
    <footer><span>{endpoints.length} connections available</span><small>SFTP reuses SSH host configuration</small></footer>
  </section>
  return <section className={`dsh-ssh-file-pane${dragOver ? ' is-drop-target' : ''}`} onDragOver={event => { if (event.dataTransfer.types.includes(REMOTE_FILES_DRAG_TYPE) && canDropIntoPane(dragSource, pane)) { event.preventDefault(); event.dataTransfer.dropEffect = dragSource?.endpointId === pane.endpointId ? 'move' : 'copy'; setDirectoryDropTarget(undefined); setDragOver(true) } else if (dragOver) setDragOver(false) }} onDragLeave={event => { if (!event.currentTarget.contains(event.relatedTarget as Node)) { setDragOver(false); setDirectoryDropTarget(undefined) } }} onDrop={drop}>
    <header><button type="button" className="dsh-ssh-pane-back" aria-label="Back to connection list" onClick={showConnections}><IconChevronLeftOutline14 size={14} /></button><span className="dsh-ssh-pane-endpoint"><strong>{endpoint.name}</strong><small title={endpoint.address}>{endpoint.address}</small></span><span className={`dsh-ssh-protocol-badge is-${endpoint.protocol}`}>{protocolShort(endpoint.protocol)}</span></header>
    <form className="dsh-ssh-file-pathbar" onSubmit={submitPath}><button type="button" aria-label="Parent directory" disabled={view?.parent === null || loading} onClick={() => { if (view?.parent) void load(view.parent) }}><UpGlyph /></button><button type="button" aria-label="Refresh directory" disabled={loading} onClick={() => { void load() }}><RefreshGlyph /></button><input aria-label="Remote path" value={draftPath} onChange={event => setDraftPath(event.target.value)} /><button type="submit" disabled={loading}>Go to</button></form>
    <div className="dsh-ssh-file-table" role="grid" aria-busy={loading}>
      <div className="dsh-ssh-file-table-head" role="row">
        <SortColumn label="Name" sortKey="name" current={sort} onSort={changeSort} />
        <SortColumn label="Size" sortKey="size" current={sort} onSort={changeSort} />
        <SortColumn label="Modified" sortKey="modifiedAt" current={sort} onSort={changeSort} />
        <span role="columnheader" aria-label="Actions" />
      </div>
      <div ref={bodyRef} className="dsh-ssh-file-table-body" onScroll={event => { if (virtualized) setScrollTop(event.currentTarget.scrollTop) }}>{loading && !view ? <div className="dsh-ssh-file-loading">Reading directory…</div> : error ? <div className="dsh-ssh-file-error"><span>{error}</span><button type="button" onClick={() => { void load() }}>Retry</button></div> : entries.length === 0 ? <div className="dsh-ssh-table-empty">This directory is empty.</div> : <div className={virtualized ? 'dsh-ssh-file-virtual-list' : undefined} style={virtualized ? { height: `${entries.length * FILE_ROW_HEIGHT}px` } : undefined}>{visibleEntries.map((entry, offset) => <FileEntryRow key={entry.path} entry={entry} paneId={pane.id} endpointId={endpoint.id} sourceDirectory={view?.path ?? pane.path} dragSource={dragSource} dropTarget={directoryDropTarget === entry.path} selected={selected.includes(entry.path)} selectedPaths={selected} {...virtualized ? { style: { position: 'absolute', insetInline: 0, transform: `translateY(${(virtualStart + offset) * FILE_ROW_HEIGHT}px)` } } : {}} onDelete={() => setDeleteTarget([entry])} onDirectoryTarget={target => { setDragOver(false); setDirectoryDropTarget(target) }} onDropIntoDirectory={(payload, target) => { setDirectoryDropTarget(undefined); onDragSourceChange(undefined); onExternalDrop(payload, target) }} onDragSourceChange={onDragSourceChange} onSelect={additive => select(entry, additive)} onOpen={() => { void load(entry.path) }} />)}</div>}</div>
    </div>
    <footer><span>{selected.length > 0 ? `Selected ${selected.length} items` : `${view?.entries.length ?? 0} items`}</span><span className="dsh-ssh-file-pane-actions"><button type="button" data-ssh-interactive="control" className="dsh-ssh-transfer-to-button" disabled={selected.length === 0 || destination === undefined || !destination.endpointId || loading} onClick={() => { if (destination?.endpointId) onTransfer(selected, destination) }}>Send to next pane <span aria-hidden="true">→</span></button></span></footer>
    {dragOver && <div className="dsh-ssh-file-drop-overlay"><strong>{dragSource?.endpointId === pane.endpointId ? "Move to this directory" : "Copy to this directory"}</strong><span>{view?.path ?? pane.path}</span></div>}
...
function FileEntryRow({ entry, paneId, endpointId, sourceDirectory, dragSource, dropTarget, selected, selectedPaths, style, onSelect, onOpen, onDelete, onDirectoryTarget, onDropIntoDirectory, onDragSourceChange }: { entry: SftpEntryView; paneId: string; endpointId: string; sourceDirectory: string; dragSource?: TransferDragSource | undefined; dropTarget: boolean; selected: boolean; selectedPaths: string[]; style?: CSSProperties | undefined; onSelect(additive: boolean): void; onOpen(): void; onDelete(): void; onDirectoryTarget(path?: string): void; onDropIntoDirectory(payload: TransferDragSource, destinationDirectory: string): void; onDragSourceChange(source?: TransferDragSource): void }): JSX.Element {
  ...
  return <div
    ...
    aria-label={`${entry.name}${directory ? ", directory, click to enter; accepts drag & drop" : ''}`}
...
function ConflictDialog({ job, onClose, onRetry }: { job: TransferJobView; onClose(): void; onRetry(policy: 'skip' | 'overwrite' | 'rename'): Promise<void> }): JSX.Element {
  ...
  return <Dialog className="dsh-ssh-transfer-conflict-dialog" title="A file with the same name exists at the destination" subtitle="Choose how to handle this batch of transfers" onClose={onClose}><div className="dsh-ssh-transfer-conflict-copy"><strong>{remoteLabel(job.request.sourcePaths)}</strong><span>{job.request.destinationDirectory}</span></div><div className="dsh-ssh-conflict-options">{([['rename', "Auto rename", "Keep both files; append a number to the name"], ['skip', "Skip files with the same name", "Continue transferring the remaining items"], ['overwrite', "Overwrite the target files", "Replace the destination content with the source content"]] as const).map(option => <button type="button" data-ssh-interactive="choice" aria-pressed={policy === option[0]} key={option[0]} className={policy === option[0] ? 'is-active' : ''} onClick={() => setPolicy(option[0])}><i aria-hidden="true" /><span><strong>{option[1]}</strong><small>{option[2]}</small></span></button>)}</div><div className="dsh-ssh-dialog-actions"><button type="button" className="dsh-ssh-secondary-button" data-ssh-dialog-close onClick={onClose}>Cancel</button><button type="button" className="dsh-ssh-primary-button" disabled={saving} onClick={() => { setSaving(true); void onRetry(policy).finally(() => setSaving(false)) }}>{saving ? "Re-queuing…" : "Continue transfer"}</button></div></Dialog>
}
...
function FileAccessDialog({ endpoints, access, onClose }: { endpoints: FileEndpointView[]; access: SessionAccessState; onClose(): void }): JSX.Element {
  ...
  return <Dialog className="dsh-ssh-file-access-dialog" title="Session access" subtitle="Manage the remote file connections available to this session" onClose={onClose}>
    <div className="dsh-ssh-file-access-body dsh-ssh-scroll-surface">
      ...
      <section className="dsh-ssh-file-access-section" aria-labelledby="dsh-ssh-file-access-connections">
        <header><span><strong id="dsh-ssh-file-access-connections">File connections</strong><small>Let the current session browse selected FTP, FTPS, or SFTP</small></span><i>{mounted.toString().padStart(2, '0')} / {endpoints.length.toString().padStart(2, '0')}</i></header>
        <div className="dsh-ssh-file-access-list">{access.loading ? <FileAccessSkeleton /> : endpoints.length === 0 ? <div className="dsh-ssh-file-access-empty"><strong>No connections available</strong><small>Add an FTP connection in File Transfer first, or add an SSH host in the hosts panel.</small></div> : endpoints.map(endpoint => <FileAccessEndpoint key={endpoint.id} endpoint={endpoint} enabled={value?.fileEndpointIds.includes(endpoint.id) ?? false} onToggle={() => toggle(endpoint.id)} />)}</div>
      </section>
      <section className="dsh-ssh-file-access-section is-options" aria-labelledby="dsh-ssh-file-access-policy">
        <header><span><strong id="dsh-ssh-file-access-policy">Access policy</strong><small>File permissions are independent of SSH command and terminal permissions</small></span></header>
        <div className="dsh-ssh-file-access-options">
          <div className="dsh-ssh-file-access-policy-row"><span><strong>File operation permission</strong><small>Browse mode does not expose cross-endpoint transfer tools to the AI</small></span><FilePermissionPicker value={value?.filePermission ?? 'browse'} onChange={access.setFilePermission} /></div>
          <div className="dsh-ssh-file-access-policy-row"><span><strong>Confirm before transfer</strong><small>Request DSH approval when the AI starts a transfer</small></span><FileApprovalSwitch checked={value?.requireFileApproval ?? true} onChange={access.setRequireFileApproval} /></div>
        </div>
      </section>
    </div>
    <footer className="dsh-ssh-file-access-footer"><span className={access.saving ? 'is-saving' : 'is-saved'}><i aria-hidden="true" />{access.saving ? "Saving changes" : `${mounted > 0 ? `Authorized ${mounted} connections` : "No authorized connections"} · autosaves`}</span><button type="button" className="dsh-ssh-primary-button" data-ssh-dialog-close disabled={access.saving} onClick={onClose}>{access.saving ? "Saving…" : "Done"}</button></footer>
  </Dialog>
}
...
function FileAccessEndpoint({ endpoint, enabled, onToggle }: { endpoint: FileEndpointView; enabled: boolean; onToggle(): void }): JSX.Element {
  return <button type="button" data-ssh-interactive="choice" className={`dsh-ssh-file-access-endpoint${enabled ? ' is-mounted' : ''}`} aria-pressed={enabled} onClick={onToggle}>
    ...
    <span className="dsh-ssh-file-access-state"><i aria-hidden="true"><b /></i><em>{enabled ? "Authorized" : "Not authorized"}</em></span>
  </button>
}

function FileAccessSkeleton(): JSX.Element {
  return <div className="dsh-ssh-file-access-skeleton" aria-label="Reading session permissions">{[0, 1, 2].map(index => <span key={index} style={{ '--ssh-skeleton-index': index } as CSSProperties}><i /><b><i /><i /></b><em /></span>)}</div>
}

function FilePermissionPicker({ value, onChange }: { value: 'browse' | 'transfer'; onChange(value: 'browse' | 'transfer'): void }): JSX.Element {
  return <div className="dsh-ssh-file-permission-picker" role="radiogroup" aria-label="File operation permissions">{([['browse', "Browse only"], ['transfer', "Allow transfer"]] as const).map(([id, label]) => <button type="button" data-ssh-interactive="choice" role="radio" aria-checked={value === id} className={value === id ? 'is-active' : ''} key={id} onClick={() => onChange(id)}>{label}</button>)}</div>
}

function FileApprovalSwitch({ checked, onChange }: { checked: boolean; onChange(value: boolean): void }): JSX.Element {
  return <button type="button" data-ssh-interactive="choice" role="switch" aria-checked={checked} className={`dsh-ssh-file-approval-switch${checked ? ' is-active' : ''}`} onClick={() => onChange(!checked)}><i aria-hidden="true"><b /></i><span>{checked ? "Confirmation required" : "Run directly"}</span></button>
}
...
function createTab(id: string, index: number): TransferTab { return { id, name: `Tasks ${index}`, panes: [createPane(`${id}-a`), createPane(`${id}-b`)] } }
...
function remoteLabel(paths: string[]): string { const name = paths[0]?.replaceAll('\\', '/').split('/').filter(Boolean).at(-1) ?? "Files"; return paths.length > 1 ? `${name} and ${paths.length} more` : name }
...
function jobLabel(job: TransferJobView, progress: number): string { if (job.state === 'queued') return "Waiting to transfer"; if (job.state === 'scanning') return "Scanning directories"; if (job.state === 'transferring') { const seconds = Math.max(1, (Date.now() - (job.startedAt ?? Date.now())) / 1000); const speed = job.transferredBytes / seconds; const remaining = speed > 0 ? Math.max(0, (job.totalBytes - job.transferredBytes) / speed) : 0; return `${progress.toFixed(0)}% · ${formatBytes(speed)}/s${remaining > 1 ? ` · ~ ${formatDuration(remaining)}` : ''}` } if (job.state === 'completed') return `Completed ${job.completedFiles} files${job.skippedFiles ? `, skipped ${job.skippedFiles}` : ''}`; if (job.state === 'cancelled') return "Cancelled"; return "Transfer failed" }
function jobTimeLabel(job: TransferJobView): string {
  if (job.startedAt === undefined) return `Created ${formatClock(job.createdAt)}`
  const end = job.completedAt ?? Date.now()
  const elapsed = formatElapsed(Math.max(0, end - job.startedAt))
  return job.completedAt === undefined ? `Executed ${elapsed}` : `Took ${elapsed} · completed ${formatClock(job.completedAt)}`
}
...
function formatElapsed(milliseconds: number): string {
  ...
  if (minutes < 60) return `${minutes} minutes ${String(seconds % 60).padStart(2, '0')} seconds`
  return `${Math.floor(minutes / 60)} hours ${String(minutes % 60).padStart(2, '0')} minutes`
}
function formatDuration(seconds: number): string { if (seconds < 60) return `${Math.ceil(seconds)} seconds`; const minutes = Math.ceil(seconds / 60); return minutes < 60 ? `${minutes} minutes` : `${Math.floor(minutes / 60)} hours ${minutes % 60} minutes` }
