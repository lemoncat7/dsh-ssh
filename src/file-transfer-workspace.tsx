...
export function FileTransferWorkspace({ ftpProfiles, vaultEntries, proxyEntries, access, onProfilesChanged }: { ftpProfiles: FtpProfileView[]; vaultEntries: VaultEntryView[]; proxyEntries: ProxyEntryView[]; access: SessionAccessState; onProfilesChanged(): void }): JSX.Element {
  ...
  return <div className="dsh-ssh-transfer-workspace">
    <header className="dsh-ssh-transfer-header">
      <div><h1>{t("client.fileTransfer")}</h1><p>{t("file-transfer-workspace.directStreamingTransfersBetweenFtpFtpsAndSftp")}</p></div>
      <div className="dsh-ssh-transfer-actions"><button type="button" data-ssh-interactive="control" className="dsh-ssh-secondary-button" onClick={() => setAccessOpen(true)}>{t("file-transfer-workspace.sessionAccess")} {access.value?.fileEndpointIds.length ?? 0}</button><button type="button" data-ssh-interactive="control" className="dsh-ssh-secondary-button" onClick={() => setConnectionsOpen(true)}>{t("file-transfer-workspace.ftpManager")}</button></div>
    </header>
    {error && <div className="dsh-ssh-banner is-error" role="alert"><span>{error}</span><button type="button" onClick={() => setError(undefined)} aria-label={t("client.close")}><IconCloseOutline16 size={16} /></button></div>}
    <section ref={stageGlow.ref} onPointerMove={stageGlow.onPointerMove} onPointerLeave={stageGlow.onPointerLeave} className="dsh-ssh-transfer-stage dsh-ssh-border-surface">
      <div className="dsh-ssh-transfer-tabbar" role="tablist" aria-label={t("file-transfer-workspace.fileTransferTasksPage")}>
        <div className="dsh-ssh-transfer-tabs">{tabs.map(tab => <span data-ssh-interactive="choice" className={`dsh-ssh-transfer-tab${tab.id === active?.id ? ' is-active' : ''}`} key={tab.id}><button type="button" role="tab" aria-selected={tab.id === active?.id} onClick={() => setActiveTabId(tab.id)}>{transferTabLabel(tab)}</button>{tabs.length > 1 && <button type="button" className="is-close" aria-label={t("file-transfer-workspace.close", [transferTabLabel(tab)])} onClick={() => closeTab(tab.id)}><IconCloseOutline16 size={13} /></button>}</span>)}<button type="button" data-ssh-interactive="control" className="is-add" aria-label={t("file-transfer-workspace.newTransferTasksPage")} onClick={addTab}><IconPlusOutline16 size={15} /></button></div>
...
function FileTransferPane({ pane, endpoints, destination, dragSource, refreshRevision, onChange, onTransfer, onExternalDrop, onDragSourceChange, onManageConnections }: { pane: PaneState; endpoints: FileEndpointView[]; destination?: PaneState; dragSource?: TransferDragSource | undefined; refreshRevision: number; onChange(patch: Partial<PaneState>): void; onTransfer(paths: string[], destination: PaneState): void; onExternalDrop(source: TransferDragSource, destinationDirectory: string): void; onDragSourceChange(source?: TransferDragSource): void; onManageConnections(): void }): JSX.Element {
  ...
  const entries = useMemo(() => sortFileEntries(view?.entries ?? [], sort.key, sort.direction), [view?.entries, sort, sshLocale])
  const virtualized = entries.length > 200
  const virtualStart = virtualized ? Math.max(0, Math.floor(scrollTop / FILE_ROW_HEIGHT) - 8) : 0
  const visibleEntries = useMemo(() => virtualized ? entries.slice(virtualStart, Math.min(entries.length, virtualStart + 56)) : entries, [entries, virtualStart, virtualized, sshLocale])
  ...
  const openEntry = async (path: string): Promise<void> => {
    if (!endpoint) return
    const generation = ++loadGenerationRef.current
    setLoading(true); setError(undefined)
    try {
      const entry = await inspectFileEndpointEntry(pane.id, endpoint.id, path)
      if (generation !== loadGenerationRef.current) return
      if (isNavigableRemoteEntry(entry)) await load(entry.path)
      else setOpenedFile(entry)
    } catch (reason) {
  ...
  if (endpoint === undefined) return <section className="dsh-ssh-file-pane is-connections">
    <header><span className="dsh-ssh-pane-heading"><strong>{t("file-transfer-workspace.connectionList")}</strong><small>{t("file-transfer-workspace.clickAConnectionToBrowseRemoteFiles")}</small></span><button type="button" data-ssh-interactive="control" className="dsh-ssh-pane-manage-button" onClick={onManageConnections}>{t("file-transfer-workspace.ftpManager")}</button></header>
    <div className="dsh-ssh-endpoint-list">{endpoints.map(item => <button type="button" data-ssh-interactive="row" key={item.id} className="dsh-ssh-endpoint-row" onClick={() => chooseEndpoint(item)}><span className={`dsh-ssh-protocol-badge is-${item.protocol}`}>{protocolShort(item.protocol)}</span><span><strong title={item.name}>{item.name}</strong><small title={item.address}>{item.address}</small></span><span><i className={`dsh-ssh-protocol-dot is-${item.protocol}`} aria-hidden="true" />{item.group ?? (item.kind === 'sftp' ? t("file-transfer-workspace.sshHosts") : t("file-transfer-workspace.fileConnections"))}</span></button>)}</div>
    <footer><span>{endpoints.length}  {t("file-transfer-workspace.availableConnections")}</span><small>{t("file-transfer-workspace.sftpReusesSshHostConfiguration")}</small></footer>
  </section>
  return <section className={`dsh-ssh-file-pane${dragOver ? ' is-drop-target' : ''}`} onDragOver={event => { if (event.dataTransfer.types.includes(REMOTE_FILES_DRAG_TYPE) && canDropIntoPane(dragSource, pane)) { event.preventDefault(); event.dataTransfer.dropEffect = dragSource?.endpointId === pane.endpointId ? 'move' : 'copy'; setDirectoryDropTarget(undefined); setDragOver(true) } else if (dragOver) setDragOver(false) }} onDragLeave={event => { if (!event.currentTarget.contains(event.relatedTarget as Node)) { setDragOver(false); setDirectoryDropTarget(undefined) } }} onDrop={drop}>
    ...
    <div className="dsh-ssh-file-table" role="grid" aria-busy={loading}>
      <div className="dsh-ssh-file-table-head" role="row">
        ...
        <SortColumn label={t("file-transfer-workspace.size")} sortKey="size" current={sort} onSort={changeSort} />
        <SortColumn label={t("file-transfer-workspace.modified")} sortKey="modifiedAt" current={sort} onSort={changeSort} />
        <span role="columnheader" aria-label={t("file-transfer-workspace.actions")} />
      </div>
    ...
    {openedFile !== undefined && <Dialog variant="confirmation" className="dsh-ssh-file-download-dialog" title={openedFile.name} subtitle={openedFile.path} onClose={() => setOpenedFile(undefined)}><div className="dsh-ssh-dialog-actions"><a className="dsh-ssh-secondary-button" href={fileEndpointDownloadUrl(endpoint.id, openedFile.path)} download><DownloadGlyph /><span>{t("file-transfer-workspace.download")}</span></a></div></Dialog>}
...
function FileEntryRow({ entry, paneId, endpointId, sourceDirectory, dragSource, dropTarget, selected, selectedPaths, style, onSelect, onOpen, onDelete, onDirectoryTarget, onDropIntoDirectory, onDragSourceChange }: { entry: SftpEntryView; paneId: string; endpointId: string; sourceDirectory: string; dragSource?: TransferDragSource | undefined; dropTarget: boolean; selected: boolean; selectedPaths: string[]; style?: CSSProperties | undefined; onSelect(additive: boolean): void; onOpen(): void; onDelete(): void; onDirectoryTarget(path?: string): void; onDropIntoDirectory(payload: TransferDragSource, destinationDirectory: string): void; onDragSourceChange(source?: TransferDragSource): void }): JSX.Element {
  ...
  return <div
    ...
    aria-label={`${entry.name}${directory ? t("file-transfer-workspace.directoryClickToEnterAcceptsDragDrop") : probeOnOpen ? t("file-transfer-workspace.clickToView") : ''}`}
  ...
  ><span><FileGlyph directory={directory} /><i title={entry.name}>{entry.name}</i></span><span>{directory ? '—' : formatBytes(entry.size)}</span><span>{entry.modifiedAt > 0 ? new Date(entry.modifiedAt).toLocaleString() : '—'}</span><span className="dsh-ssh-file-row-actions dsh-ssh-context-action">{<a className="dsh-ssh-file-row-download" href={fileEndpointDownloadUrl(endpointId, entry.path)} download draggable={false} aria-label={t("file-transfer-workspace.downloadLocally", [entry.name])} title={entry.kind === 'directory' ? t("file-transfer-workspace.packageAndDownloadLocally", [entry.name]) : t("file-transfer-workspace.downloadLocally", [entry.name])} onClick={event => event.stopPropagation()}><DownloadGlyph /></a>}<button type="button" className="dsh-ssh-file-row-delete" draggable={false} aria-label={t("client.delete2", [entry.name])} title={t("client.delete2", [entry.name])} onClick={event => { event.stopPropagation(); onDelete() }}><IconTrashOutline16 size={14} /></button></span></div>
}
...
function ConflictDialog({ job, onClose, onRetry }: { job: TransferJobView; onClose(): void; onRetry(policy: 'skip' | 'overwrite' | 'rename'): Promise<void> }): JSX.Element {
  ...
  return <Dialog variant="confirmation" className="dsh-ssh-transfer-conflict-dialog" title={t("file-transfer-workspace.aFileWithTheSameNameExistsAtThe")} subtitle={t("file-transfer-workspace.chooseHowToHandleThisBatchOfTransfers")} onClose={onClose}><div className="dsh-ssh-transfer-conflict-copy"><strong>{remoteLabel(job.request.sourcePaths)}</strong><span>{job.request.destinationDirectory}</span></div><div className="dsh-ssh-conflict-options">{([['rename', t("file-transfer-workspace.autoRename"), t("file-transfer-workspace.keepBothFilesAppendANumberToTheName")], ['skip', t("file-transfer-workspace.skipFilesWithTheSameName"), t("file-transfer-workspace.continueTransferringTheRemainingItems")], ['overwrite', t("file-transfer-workspace.overwriteTheTargetFiles"), t("file-transfer-workspace.replaceTheDestinationContentWithTheSourceContent")]] as const).map(option => <button type="button" data-ssh-interactive="choice" aria-pressed={policy === option[0]} key={option[0]} className={policy === option[0] ? 'is-active' : ''} onClick={() => setPolicy(option[0])}><i aria-hidden="true" /><span><strong>{option[1]}</strong><small>{option[2]}</small></span></button>)}</div><div className="dsh-ssh-dialog-actions"><button type="button" className="dsh-ssh-secondary-button" data-ssh-dialog-close onClick={onClose}>{t("client.cancel")}</button><button type="button" className="dsh-ssh-primary-button" disabled={saving} onClick={() => { setSaving(true); void onRetry(policy).finally(() => setSaving(false)) }}>{saving ? t("file-transfer-workspace.reQueuing") : t("file-transfer-workspace.continueTransfer")}</button></div></Dialog>
}
...
function FileAccessDialog({ endpoints, access, onClose }: { endpoints: FileEndpointView[]; access: SessionAccessState; onClose(): void }): JSX.Element {
  ...
  return <Dialog className="dsh-ssh-file-access-dialog" title={t("file-transfer-workspace.sessionAccess2")} subtitle={t("file-transfer-workspace.manageTheRemoteFileConnectionsAvailableToThisSession")} onClose={onClose}>
    <div className="dsh-ssh-file-access-body dsh-ssh-scroll-surface">
      ...
      <section className="dsh-ssh-file-access-section" aria-labelledby="dsh-ssh-file-access-connections">
        <header><span><strong id="dsh-ssh-file-access-connections">{t("file-transfer-workspace.fileConnections")}</strong><small>{t("file-transfer-workspace.letTheCurrentSessionBrowseSelectedFtpFtpsOr")}</small></span><i>{mounted.toString().padStart(2, '0')} / {endpoints.length.toString().padStart(2, '0')}</i></header>
        <div className="dsh-ssh-file-access-list">{access.loading ? <FileAccessSkeleton /> : endpoints.length === 0 ? <div className="dsh-ssh-file-access-empty"><strong>{t("file-transfer-workspace.noConnectionsAvailable")}</strong><small>{t("file-transfer-workspace.addAnFtpInFileTransferFirstOrAdd")}</small></div> : endpoints.map(endpoint => <FileAccessEndpoint key={endpoint.id} endpoint={endpoint} enabled={value?.fileEndpointIds.includes(endpoint.id) ?? false} onToggle={() => toggle(endpoint.id)} />)}</div>
      </section>
      <section className="dsh-ssh-file-access-section is-options" aria-labelledby="dsh-ssh-file-access-policy">
        <header><span><strong id="dsh-ssh-file-access-policy">{t("file-transfer-workspace.accessPolicy")}</strong><small>{t("file-transfer-workspace.filePermissionsAreSeparateFromSshCommandAndTerminal")}</small></span></header>
    ...
    <footer className="dsh-ssh-file-access-footer"><span className={access.saving ? 'is-saving' : 'is-saved'}><i aria-hidden="true" />{access.saving ? t("file-transfer-workspace.savingChanges") : t("file-transfer-workspace.autosaves", [mounted > 0 ? t("file-transfer-workspace.authorizedConnections", [mounted]) : t("file-transfer-workspace.noAuthorizedConnections")])}</span><button type="button" className="dsh-ssh-primary-button" data-ssh-dialog-close disabled={access.saving} onClick={onClose}>{access.saving ? t("client.saving2") : t("file-transfer-workspace.done")}</button></footer>
  </Dialog>
}
...
function FileAccessEndpoint({ endpoint, enabled, onToggle }: { endpoint: FileEndpointView; enabled: boolean; onToggle(): void }): JSX.Element {
  ...
  return <button type="button" data-ssh-interactive="choice" className={`dsh-ssh-file-access-endpoint${enabled ? ' is-mounted' : ''}`} aria-pressed={enabled} onClick={onToggle}>
    ...
    <span className="dsh-ssh-file-access-state"><i aria-hidden="true"><b /></i><em>{enabled ? t("file-transfer-workspace.authorized") : t("file-transfer-workspace.notAuthorized")}</em></span>
  </button>
}

function FileAccessSkeleton(): JSX.Element {
  useSshLocale()
  return <div className="dsh-ssh-file-access-skeleton" aria-label={t("file-transfer-workspace.readingSessionPermissions")}>{[0, 1, 2].map(index => <span key={index} style={{ '--ssh-skeleton-index': index } as CSSProperties}><i /><b><i /><i /></b><em /></span>)}</div>
}

function FilePermissionPicker({ value, onChange }: { value: 'browse' | 'transfer'; onChange(value: 'browse' | 'transfer'): void }): JSX.Element {
  useSshLocale()
  return <div className="dsh-ssh-file-permission-picker" role="radiogroup" aria-label={t("file-transfer-workspace.fileOperationPermissions")}>{([['browse', t("file-transfer-workspace.browseOnly")], ['transfer', t("file-transfer-workspace.allowTransfer")]] as const).map(([id, label]) => <button type="button" data-ssh-interactive="choice" role="radio" aria-checked={value === id} className={value === id ? 'is-active' : ''} key={id} onClick={() => onChange(id)}>{label}</button>)}</div>
}

function FileApprovalSwitch({ checked, onChange }: { checked: boolean; onChange(value: boolean): void }): JSX.Element {
  useSshLocale()
  return <button type="button" data-ssh-interactive="choice" role="switch" aria-checked={checked} className={`dsh-ssh-file-approval-switch${checked ? ' is-active' : ''}`} onClick={() => onChange(!checked)}><i aria-hidden="true"><b /></i><span>{checked ? t("file-transfer-workspace.confirmationRequired") : t("file-transfer-workspace.runDirectly")}</span></button>
}
...
function transferTabLabel(tab: TransferTab): string {
  // Keep the existing persisted representation; only translate generated display labels.
  const number = /^任务 (\d+)$/.exec(tab.name)?.[1]
  return number === undefined ? tab.name : t('file-transfer-workspace.tasks', [number])
}
...
function remoteLabel(paths: string[]): string { const name = paths[0]?.replaceAll('\\', '/').split('/').filter(Boolean).at(-1) ?? t("file-transfer-workspace.files"); return paths.length > 1 ? t("file-transfer-workspace.andMore", [name, paths.length]) : name }
...
function jobTimeLabel(job: TransferJobView): string {
  if (job.startedAt === undefined) return t("file-transfer-workspace.created", [formatClock(job.createdAt)])
  const end = job.completedAt ?? Date.now()
  const elapsed = formatElapsed(Math.max(0, end - job.startedAt))
  return job.completedAt === undefined ? t("file-transfer-workspace.executed", [elapsed]) : t("file-transfer-workspace.tookCompleted", [elapsed, formatClock(job.completedAt)])
}
function formatClock(timestamp: number): string { return transferClockFormatter.format(timestamp) }
function formatElapsed(milliseconds: number): string {
  const seconds = Math.max(0, Math.floor(milliseconds / 1000))
  if (seconds < 60) return t("file-transfer-workspace.seconds", [seconds])
  const minutes = Math.floor(seconds / 60)
  if (minutes < 60) return t("file-transfer-workspace.minutesSeconds", [minutes, String(seconds % 60).padStart(2, '0')])
  return t("file-transfer-workspace.hoursMinutes", [Math.floor(minutes / 60), String(minutes % 60).padStart(2, '0')])
}
function formatDuration(seconds: number): string { if (seconds < 60) return t("file-transfer-workspace.seconds", [Math.ceil(seconds)]); const minutes = Math.ceil(seconds / 60); return minutes < 60 ? t("file-transfer-workspace.minutes", [minutes]) : t("file-transfer-workspace.hMin", [Math.floor(minutes / 60), minutes % 60]) }
