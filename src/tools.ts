...
function terminalOpenTool(store: SshStore, terminals: AiTerminalManager): ToolDefinition {
  return tool('ssh_terminal_open', "Open or reuse an interactive terminal on an SSH connection injected with terminal permission. Do not open a terminal for file upload, download, or transfer; use the dedicated file_* tools or direct the user to SSH → File Transfer. Repeated calls for the same conversation, connection, and working directory are idempotent; exited duplicates are removed automatically. Returns an owner-scoped terminal id and whether it was reused.", {
    profileId: { type: 'string', required: true, description: 'Exact injected profile id returned by ssh_list.' },
...
function requireInjection(store: SshStore, exec: ToolRunContext) {
  ...
  const injection = store.injection(sessionId)
  if (injection === undefined || injection.profileIds.length === 0) throw new Error("No SSH connection is injected into this DSH session. Ask the user to open Remote and inject one.")
  return injection
}
