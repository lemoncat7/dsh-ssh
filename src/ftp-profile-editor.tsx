        ...
        {profiles.length === 0 ? <div className="dsh-ssh-ftp-list-empty"><span><IconDataOutline16 size={20} /></span><strong>还没有独立文件连接</strong><p>SFTP 会显示在文件窗格的连接列表中；这里只管理 FTP 与 FTPS。</p><button type="button" className="dsh-ssh-secondary-button" onClick={create}>新建 FTPS 连接</button></div> : <div className="dsh-ssh-ftp-profile-list" role="list">{profiles.map(profile => {
          ...
          return <article key={profile.id} role="listitem" data-ssh-interactive="choice" aria-selected={active} className={active ? 'is-active' : ''}>
            <button type="button" className="dsh-ssh-ftp-profile-main" onClick={() => select(profile)}>
              ...
              <i className={(profile.authMode === 'anonymous' || profile.credential.configured) ? 'is-ready' : 'is-missing'}>{profile.authMode === 'anonymous' ? '匿名登录' : profile.credential.configured ? '可用' : '缺少密码'}</i>
            </button>
...
function FtpProfileEditor({ value, profiles, vaultEntries, proxyEntries, onBack, onSaved }: { value?: FtpProfileView | undefined; profiles: FtpProfileView[]; vaultEntries: VaultEntryView[]; proxyEntries: ProxyEntryView[]; onBack(): void; onSaved(): void }): JSX.Element {
  ...
  return <form className="dsh-ssh-ftp-editor-form" onSubmit={event => { void submit(event) }}>
    ...
    <div className="dsh-ssh-ftp-editor-scroll">
      ...
      <section className="dsh-ssh-ftp-editor-section"><div className="dsh-ssh-form-section-heading"><strong>身份认证</strong><small>密码只写入 DSH 凭据服务，不会在界面回显</small></div><Field label="登录方式"><select value={form.authMode} onChange={event => { setForm({ ...form, authMode: event.target.value as 'password' | 'anonymous' }); setTestResult(undefined) }}><option value="password">账号密码</option><option value="anonymous">匿名登录</option></select></Field>{form.authMode === 'anonymous' ? <p>使用 anonymous 登录，无需填写账号密码；服务器必须开放匿名访问。</p> : <><div className="dsh-ssh-form-grid"><Field label="用户名"><input required maxLength={128} autoComplete="username" value={form.username} onChange={event => setForm({ ...form, username: event.target.value })} /></Field><Field label="凭据来源"><select value={form.credentialId} onChange={event => setForm({ ...form, credentialId: event.target.value })}><option value="">此连接单独保存密码</option>{passwordEntries.map(entry => <option key={entry.id} value={entry.id}>{entry.name} · {entry.username}</option>)}</select></Field></div>{!form.credentialId && <Field label="密码" hint={value?.credential.configured && value.credential.source === 'profile' ? '已保存；留空保持不变' : '保存后不会回显'}><input required={value === undefined || value.credential.source !== 'profile' || !value.credential.configured} type="password" autoComplete="new-password" value={form.password} onChange={event => setForm({ ...form, password: event.target.value })} /></Field>}</>}</section>
